...
host_discovery:
  ...
  fallback_static:
    - name: gpu-1
      url: http://h:9000
      token: t
      models: [{id: m1, task: __TASK__, kind: opensource, runner: generic}]
"""
...
def _settings(tmp_path: Path, body: str) -> __SETTINGS__:
    path = tmp_path / "config.yaml"
    path.write_text(body, encoding="utf-8")
    return __SETTINGS__(service_name="__SLUG__", hosts_path=path)
...
def test_missing_config_file_gives_an_empty_static_registry(tmp_path):
    settings = __SETTINGS__(service_name="__SLUG__", hosts_path=tmp_path / "khong-co.yaml")
